...
public class RecognizerService2 extends Service {
        ...
        public void run() {
            while (mRunRecognizerThread) {
                ...
                if (strokes > 0 && mHandler != null) {
                    // call recognizer
                    String result = null;
                    result = WritePadManager.recoInkData(strokes, false, false,
                            false);
                    if (result != null) {
                       ...
                       /* int words = WritePadManager.recoResultColumnCount();
                        ...
                        { int alternatives  = WritePadAPI.recoResultRowCount(w);
                            ...
                            { String word =  WritePadAPI.recoResultWord( w, a );
                                // 						  word					  alternative
                                int weight = WritePadAPI.recoResulWeight(w, a);
                                // probability of alternative (51...100)
                                int str = WritePadAPI.recoResultStrokesNumber( w, a );
                                //						  number of strokes used for this alternative
                                Log.d("recoResult", String.format("%s  %d   %d", word, weight, str)); } }*/
                    }
                }
            }
            // Done with our work... stop the service!
            RecognizerService2.this.stopSelf();
        }
    };
    private int kind = 0;
    ...
    public IBinder onBind(Intent intent) {
        Bundle dun = intent.getExtras();
        activename = dun.getString("name");
    ...
    public class RecognizerBinder extends Binder {
        RecognizerService2 getService() {
            return RecognizerService2.this;
        }
